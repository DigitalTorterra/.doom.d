...
*** CRM
One way I've wanted to expand the capabilities of ~org-contacts~ was to include some sort of CRM (Customer Relationship Management) capabilities. We can represent our desired behavior by adding a few different properties and tags to our existing ~contacts.org~ file.
...
The functions below can be used to parse an Org file for the global ~CONTACTS-PER-WEEK~ attribute.
...
;; Get the global rate from the top
(defun org-element-keyword-get-item (item body)
  "Parses an Org-mode keyword some attribute"
  (defun sub-f (body2)
    (if (eq (car body2) item)
        (list-next body2)
        (sub-f (cdr body2))))
  (sub-f (list-next body)))

(defun org-element-keyword-get-key (body) (org-element-keyword-get-item :key body))
(defun org-element-keyword-get-value (body) (org-element-keyword-get-item :value body))

(defun org-crm-find-element-rate (body)
  "Check if a keyword matches `org-crm-global-rate` and get value if so"
  (if (string-equal org-crm-global-rate (org-element-keyword-get-key body))
      (org-element-keyword-get-value body)
      nil))

(defun org-crm-get-rate ()
  "Parse a buffer to get the CRM rate"
  (let ((res (org-element-map (org-element-parse-buffer)
                'keyword (lambda (x) (org-crm-find-element-rate x)))))
    (if res
        (string-to-number (car res))
        nil)))
#+end_src

Our next job will be to parse the entire file. The goal is to figure out who we need to contact. The way we will do that is:

1. Find all contacts with the ~CONTACT-FREQUENCY~ property, which marks them as being people we are interested in contacting
2. Sort these contacts according to ~CONTACT-NEXT-DATE~, with uninitialized contaacts at the front and ~CONTACT-FREQUENCY~ being a secondary key
3. Choose the first ~CONTACTS-PER-WEEK~ contacts and return them
