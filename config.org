...
#+begin_src emacs-lisp
(setq org-directory "/home/pfaley2/org/")
#+end_src
...
#+begin_src emacs-lisp
;;; Functions for parsing org buffer
...
(defun assoc-get-ordered-line (alist keys)
  ...
  (mapcar (lambda (key)
            (string-clean-whitespace (cdr (assoc key alist))))
          keys))
...
(defun csv-write-line (line)
  ...
  (insert (concat
           (string-join (mapcar 'sanitize-newline line) ",")
           "\n")))

;; TODO: standardize csv delimeter (maybe do TSV instead of CSV) and strip out from text
...
(defun org-drill-export-to-csv ()
  ...
  (let ((body (org-element-parse-buffer))
        (data (org-drill-export-parse-buffer body)))
    data))
#+end_src
