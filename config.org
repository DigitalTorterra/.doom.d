...
#+begin_src emacs-lisp
(setq org-directory "/Users/p870220/org/")
(setq org-agenda-files (list (concat org-directory "todo/8451.org")))
#+end_src

#+RESULTS:
| /Users/p870220/org/todo/8451.org |
...
** Contact Management
*** ~org-contacts~
I've been experimenting with using the ~org-contacts~ package to manage my contacts. It includes the ability to apply common tags for things like name, email, and number, as well as useful information like birthdays and just general notes. The following code block installs the package. I think it used to come pre-bundled, but that seems to no longer be the case.

#+begin_src emacs-lisp :tangle packages.el
(package! org-contacts)
#+end_src

The following code block configures the package to use our specified contacts file.
...
** Postman Knock-off
The [[https://github.com/federicotdn/verb][verb]] package enables org mode to act as a Postman replacement.

#+begin_src emacs-lisp :tangle packages.el
(package! verb)
#+end_src

This command adds a sub-menu in the Org branch of SPC keybindings.

#+begin_src emacs-lisp
(map! :leader
    :desc "Verb Requests" "m v" #'verb-command-map)
#+end_src

** Custom Agenda View
#+begin_src emacs-lisp
(defun org-cmp-breadcrumb (a b)
  "Compare agenda items by their full outline path (breadcrumb)."
  (let* ((breadcrumb-a (or (get-text-property 0 'breadcrumbs a) ""))
         (breadcrumb-b (or (get-text-property 0 'breadcrumbs b) "")))
    (cond
     ((string< breadcrumb-a breadcrumb-b) -1)
     ((string< breadcrumb-b breadcrumb-a) 1)
     (t 0))))

(setq org-agenda-cmp-user-defined 'org-cmp-breadcrumb)

(setq org-agenda-custom-commands
  '(("d" "Priority Tasks"
     alltodo ""
     ((org-agenda-overriding-header "📋 Priority Tasks")
      (org-agenda-prefix-format "  %i %-50b")
      (org-agenda-sorting-strategy '(priority-down todo-state-up user-defined-up))
      (org-agenda-skip-function '(org-agenda-skip-entry-if 'notregexp "\\[#[A-Z]\\]"))))))
#+end_src
...
* LLMs
** GitHub Copilot
In this section, we attempt to include GitHub Copilot support. The snippet below installs the library.
...
Additionally, the below code defines some useful keybindings.

#+begin_src emacs-lisp
; (setq copilot-node-executable "/home/pfaley/.nvm/versions/node/v20.11.1/bin/npm")

;; accept completion from copilot and fallback to company
(use-package! copilot
  :hook (prog-mode . copilot-mode)
  ;; :config (customize-set-variable 'copilot-node-executable
;            "/home/pfaley/.nvm/versions/node/v20.11.1/bin/npm")
...
#+end_src
** ~gptel~ (for Chat)
We use the [[https://github.com/karthink/gptel?tab=readme-ov-file#doom-emacs][gptel]] package to integrate with the LLM. We also use the [[https://github.com/diasjorge/emacs-load-env-vars][load-env-vars]] package to read the endpoint location from a ~.env~ file.
...
Our first step is loading the API key from an environment file.

#+begin_src emacs-lisp
(setq proxy-env-path "~/.env")

(load-env-vars proxy-env-path)
(setq openai-api-key
      (getenv "MODEL_PROXY_API_KEY"))
#+end_src
...
(setq
 gptel-model 'gpt-4o
 gptel-backend
 (gptel-make-openai "AI Model Proxy"
   :host "api-internal.8451.com/ai/proxy"
   :key #'gptel-api-key
...
** ~aidermacs~
The [[https://github.com/MatthewZMD/aidermacs][aidermacs]] package integrate the Aider LLM tool into Emacs.
...
(use-package! aidermacs
 :config
 (setenv "AZURE_API_KEY" openai-api-key)
 (setenv "AZURE_API_VERSION" "2023-12-01-preview")
 (setenv "AZURE_API_BASE" "https://api-internal.8451.com/ai/proxy/")

 :custom
 (aidermacs-default-chat-mode 'architect)
 (aidermacs-default-model "azure/gpt-4o"))
#+end_src

** ~claude-code-ide~
[[https://github.com/manzaltu/claude-code-ide.el][claude-code-ide]] provides Claude Code integration for Emacs.
...
(use-package! claude-code-ide
  ...
  :config
  ;; DEBUGGING: Disable vterm anti-flicker optimization to test if this causes the redraw issue
  (setq claude-code-ide-vterm-anti-flicker nil)
  (claude-code-ide-emacs-tools-setup)) ; Optionally enable Emacs MCP tools

(map! :leader
 :desc "Claude" "d" #'claude-code-ide-menu)
#+end_src
...
** Format Shortcuts
This section is for random ideas I've had to make my life marginally easier.

*** Markdown to Python Lists
The idea here is that we'd want to convert a bulleted list like this:

- apple
- bannana
- cherry

to a Python list of the form:

#+sbegin_src python :tangle no
[
    'apple',
    'banana',
    'cherry',
...
The function to do that is below.

#+begin_src emacs-lisp
(defun convert-list-to-python (start end)
  "Print number of lines and characters in the region."
  (interactive "r")
  (kill-new
    (let* ((bullets (buffer-substring start end))
           (split_list (split-string bullets "\n"))
           (split_list_filtered (
               seq-filter
           ...
           (split_list_padded (
                ...
                split_list_cleaned)))
      (concat "[\n" (apply 'concat split_list_padded) "]"))))
#+end_src

#+RESULTS:
: convert-list-to-python
** Mermaid Diagrams
We rely on the ~mermaid_mode~ project.

#+begin_src emacs-lisp :tangle packages.el
(package! mermaid-mode)
#+end_src
** EPub Reading
We set up [[https://depp.brause.cc/nov.el/][nov.el]].
